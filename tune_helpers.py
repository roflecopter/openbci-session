...
TUNE_KEYS = OrderedDict([
    ...
    ('ext_recovery_ms',  (0x02, '<H',   8000,   (1,     60000))),
    ('ext_chunk_ms',     (0x03, '<H',   500,    (10,    5000))),
...
def parse_tune_arg(arg):
    ...
    Used both by argparse type= callback AND by yml-loaded tune-block
    merging so the validation lives in one place.
    """
    if '=' not in arg:
    ...
    try:
        ...
        raise ValueError(f"tune {key} value must be integer, got: {val_s!r}")
...
def merge_tune(*sources):
    ...
    Each source is a dict (or anything dict-like) of {name: int}; unknown
    keys raise ValueError. Returns a fresh OrderedDict with all 5 keys
    populated, defaults filled in for any source that omitted them.
    ...
    for src in sources:
        ...
        for k, v in src.items():
            ...
            try:
                ...
                raise ValueError(
                    f"tune {k} value must be integer, got: {v!r}")
            if vv < lo or vv > hi:
                raise ValueError(
